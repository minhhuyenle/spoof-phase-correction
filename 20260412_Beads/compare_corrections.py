...
import numpy as np
from scipy.fft import fft2, fftshift, ifft2, ifftshift, dctn, idctn
import matplotlib
matplotlib.use("Agg")
...
N_ZERNIKE = 21   # number of Zernike modes (matches your notebook)
...
# ── Method 2: Zernike fit (from your notebook) ───────────────────────────────
...
def fit_zernike(unwrapped: np.ndarray, circ: np.ndarray,
    ...
    """Fit Zernike polynomials by least-squares (your notebook approach)."""
    size  = unwrapped.shape[0]
...
def run():
    ...
    data_path = args.data
    out_dir   = data_path.parent / "python_results"
    ...
    # Average all frames for a cleaner phase estimate
    ...
    # Circular aperture (same as friend's Win = RHO^2 <= 1)
    ...
    for label, field in [("P1", p1_avg), ("P2", p2_avg)]:
        ...
        # Quadratic fit — subtract surface directly in unwrapped domain (your notebook approach)
        print(f"  Fitting quadratic ({label})...")
        quad_mask, quad_coeffs = fit_quadratic(unwrapped, circ)
        quad_residual = unwrapped - quad_mask           # direct subtraction, no re-wrapping
        quad_residual[~circ] = np.nan
        quad_corrected = np.angle(np.exp(-1j * quad_mask) * field)   # complex field for display only

        # Zernike fit — same direct subtraction
        print(f"  Fitting {N_ZERNIKE} Zernike modes ({label})...")
        zern_mask, zern_coeffs = fit_zernike(unwrapped, circ, N_ZERNIKE)
        zern_residual = unwrapped - zern_mask           # direct subtraction, no re-wrapping
        zern_residual[~circ] = np.nan
        zern_corrected = np.angle(np.exp(-1j * zern_mask) * field)   # complex field for display only
    ...
    fig.suptitle("Quadratic vs Zernike phase correction — averaged over all frames", fontsize=13)
    ...
    fig.suptitle("How much aberration each method removes", fontsize=12)

    for ai, label in enumerate(["P1", "P2"]):
        r = results[label]
        std_raw  = np.nanstd(dct_unwrap(r["raw_phase"])[circ])
    ...
    fig.suptitle(f"Fitted Zernike coefficients ({N_ZERNIKE} modes)", fontsize=12)
    ...
    # Shade the 6 quadratic-equivalent terms (tilt, defocus, astigmatism)
    quad_equiv = [1, 2, 3, 4, 5]   # Zernike indices that overlap with quadratic
